...
if __name__ == '__main__':
    ...
    parts = [p for s in soup('score-partwise') for p in s('part')]
    measures = [m for p in parts for m in p('measure')]
    print 'number of parts: %d' % len(parts)
    print 'number of measures: %d' % len(measures)
    notes = [m.select('note') for m in measures]
    print 'number of notes in first measure %d' % len(notes[0])
    print len(notes[0])
    print extract_measure_attributes(measures[0])
    for n in notes[0]:
        print note_to_string(n)
